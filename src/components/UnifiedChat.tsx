...
const EVENT_TYPES = ['Conference', 'Burn', 'ETF', 'Fork', 'Listings'];
export const UnifiedChat = () => {
  ...
  return <div className="flex flex-col h-[75vh] sm:h-[70vh] space-y-3 sm:space-y-4 max-w-4xl mx-auto px-2 sm:px-4 w-full">
      ...
      <div className="text-center space-y-1 sm:space-y-2 py-1 sm:py-2">
        <h1 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-blue-400 via-orange-500 to-amber-400 bg-clip-text text-white">CryptoBrain</h1>
      ...
      <Tabs value={activeTab} onValueChange={value => setActiveTab(value as keyof typeof TAB_CONFIG)} className="w-full">
        <TabsList className="grid w-full grid-cols-2 bg-gray-800/50 border border-gray-700 h-auto">
          ...
          return <TabsTrigger key={key} value={key} className="flex flex-col items-center gap-1 p-2 sm:p-3 text-gray-300 data-[state=active]:text-white data-[state=active]:bg-gray-700 min-h-[50px] sm:min-h-[60px] text-center">
            ...
            <Card className="flex-1 flex flex-col bg-gray-900/50 backdrop-blur-sm border-gray-700 w-full h-full">
              <CardHeader className="pb-2 sm:pb-3 border-b border-gray-700 px-3 sm:px-6 py-3 sm:py-6">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
                  ...
                  {key === 'alpha-calendar' && <div className="flex flex-col items-start sm:items-end gap-2 w-full sm:w-auto">
                      <div className="text-xs sm:text-sm font-semibold text-orange-400">Event Types</div>
                      <div className="flex flex-wrap gap-1 justify-start sm:justify-end">
                        {EVENT_TYPES.map(type => <span key={type} className="px-2 py-1 bg-orange-500/20 text-orange-300 rounded text-xs">
                            {type}
                          </span>)}
                      </div>
                    </div>}
              ...
              <CardContent className="flex-1 flex flex-col p-2 sm:p-4 min-h-0">
                <ScrollArea className="flex-1 pr-1 sm:pr-2 mb-3 sm:mb-4">
                  <div className="space-y-2 sm:space-y-3 px-1">
                    ...
                    {loading && activeTab === key && <div className="flex gap-2 sm:gap-4 justify-start">
