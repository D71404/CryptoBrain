...
const SECTION_CONFIG = {
  news: {
    ...
    examples: ["What's the latest Bitcoin news?", "Show me recent Ethereum updates", "Any news about DeFi protocols?", "Latest crypto market headlines"]
  },
  stats: {
    icon: BarChart3,
    title: 'Stats',
    ...
    examples: ["Show Bitcoin price chart", "What's the market cap of Ethereum?", "Compare top 10 cryptocurrencies", "Show trading volume for today"]
  },
  knowledge: {
    icon: BrainCog,
    title: 'Knowledge',
    ...
    examples: ["What is DeFi and how does it work?", "Explain Bitcoin's consensus mechanism", "What are the risks of yield farming?", "How do smart contracts work on Ethereum?"]
  },
  'smart-search': {
    icon: Search,
    title: 'Smart Search',
    ...
    examples: ["Search for Uniswap documentation", "Find information about Solana staking", "Look up Chainlink price feeds", "Search for NFT marketplace guides"]
  }
};
export const UnifiedChat = () => {
      ...
      if (webhookResponse.success) {
        ...
        if (response.success) {
          if (activeSection === 'knowledge') {
            ...
            botContent = articles.length > 0 ? `Found ${articles.length} recent crypto news articles:\n\n${articles.map((article: any, i: number) => `${i + 1}. ${article.title}\n${article.description}\nSource: ${article.url}\n`).join('\n')}` : 'No news articles found for your query.';
          } else if (activeSection === 'stats') {
            const stats = response.data || [];
            botContent = stats.length > 0 ? `Found crypto market data:\n\n${stats.map((stat: any, i: number) => `${i + 1}. ${stat.coinName}\nSource: ${stat.url}\n`).join('\n')}` : 'No crypto stats found for your query.';
          } else if (activeSection === 'smart-search') {
            const results = response.data?.slice(0, 5) || [];
            botContent = results.length > 0 ? `Found ${results.length} crypto-related search results:\n\n${results.map((result: any, i: number) => `${i + 1}. ${result.title}\n${result.snippet}\nSource: ${result.link}\n`).join('\n')}` : 'No search results found for your query.';
          }
        } else {
  ...
  return <div className="flex flex-col h-[70vh] space-y-2 max-w-4xl mx-auto px-4">
        ...
        <CardHeader className="pb-2 border-b border-gray-700">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-1">
              ...
              return <Button key={key} variant={isActive ? "default" : "outline"} size="sm" className={`${isActive ? `${config.color} ${config.hoverColor} text-white` : `border-gray-600 text-gray-300 hover:bg-gray-700 hover:${config.textColor.replace('text-', 'border-')}`} transition-all duration-300`} onClick={() => setActiveSection(key as keyof typeof SECTION_CONFIG)}>
        ...
        <CardContent className="flex-1 flex flex-col p-3">
          <ScrollArea className="flex-1 pr-3">
            <div className="space-y-4">
              {messages.length === 0 && <div className="text-center py-2">
                  <div className="mb-2">
                  ...
                  <div className="grid gap-1 max-w-2xl mx-auto">
                    ...
                    <div className="grid md:grid-cols-2 gap-1">
                      {activeSectionConfig.examples.map((example, index) => <button key={index} onClick={() => handleExampleClick(example)} className={`group relative text-left p-2 rounded-md border transition-all duration-300 
              ...
              {messages.map(message => <div key={message.id} className={`flex gap-4 ${message.isUser ? 'justify-end' : 'justify-start'}`}>
                  <div className={`flex gap-4 max-w-[80%] ${message.isUser ? 'flex-row-reverse' : 'flex-row'}`}>
                    <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${message.isUser ? 'bg-blue-600' : 'bg-gray-700'}`}>
                      {message.isUser ? <User className="w-5 h-5 text-white" /> : <Bot className="w-5 h-5 text-white" />}
                    </div>
                    <div className={`rounded-2xl px-4 py-3 ${message.isUser ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-100 border border-gray-700'}`}>
              ...
              {loading && <div className="flex gap-4 justify-start">
      ...
      <div className="w-full">
        <PromptInputBox onSend={handleSendMessage} isLoading={loading} placeholder={`Ask anything about ${activeSectionConfig.title.toLowerCase()}...`} activeSection={activeSection} />
      </div>
    </div>;
};